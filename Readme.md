...
## 快速开始
### -1. 前言
 搞这个程序,仅仅是我捡垃圾`k20p手机+ubuntu26`，看着它性能这么强` 8核 + 12G + 512G`，所以就想着搞PVE的，但是不支持，
 
 就自己折腾lxc，又懒开ssh所以,搞着搞着就搞成这样了。
 
 为了安全起见，务必修改用户名和密码，用户名***不要*** 用 `root/admin` ，端口尽量不要直接暴露在公网。使用nginx用域名访问就安全了。

 密码错了5次就要等5分钟，所以，爆破密码，基本上要10年8年的。
