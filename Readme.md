...
- 集中管理 , 域名穿透(类CF ZeroTrust,含服务器/客户端)
...
+ 安全： 这个工具是自带宿主机终端的，有所有权限，***无需root的账号密码***，
  ...
  管理入口默认是 /，务必在设置换成其他的，例如 `/your-admin`

+ 爆破：密码错了5次就要等5分钟，所以，爆破密码，基本上要10年8年的。
+ 开源：本系统自用，***不开源***，原因很简单，不想被扒漏洞被入侵。
### -1.0 漏洞
...
## 如果curl提示openssl错误，执行下面指令更新openssl
## apt update && apt install -y openssl && yes | pkg upgrade
```
